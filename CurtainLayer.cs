...
namespace SharpMap.Layers
{
    ...
    /// </summary>
    [Serializable]
    public class CurtainTileLayer : TileLayer, ITileAsyncLayer
    {

        #region 分隔栏
        //why the methon of drawing image is static???
        public static Point BarLocation { get; set; }
        public static Color BarColor { get; set; }
        public static int BarWidth { get; set; }
        #endregion
        ...
        public CurtainTileLayer(ITileSource tileSource, string layerName)
        ...
        public CurtainTileLayer(ITileSource tileSource, string layerName, Color transparentColor, bool showErrorInTile)
        ...
        public CurtainTileLayer(ITileSource tileSource, string layerName, Color transparentColor, bool showErrorInTile, string fileCacheDir)
        ...
        public CurtainTileLayer(ITileSource tileSource, string layerName, Color transparentColor, bool showErrorInTile, FileCache fileCache, ImageFormat imgFormat)
        ...
        void HandleMapNewTileAvaliable(Map map, Graphics g, Envelope box, Bitmap bm, int sourceWidth, int sourceHeight, ImageAttributes imageAttributes)
        {
            ...
            try
            {
                ...
                int startDrawX = BarLocation.X + BarWidth;
                switch(CT)
                {
                    case CurtainType.Right:
                        if (max.X > startDrawX)
                        {
                            int imgWidth = (int)(max.X - Math.Max((int)min.X, startDrawX));//sourcewidth
                            Rectangle destRect = new Rectangle(Math.Max((int)min.X, startDrawX), (int)max.Y, imgWidth, (int)(min.Y - max.Y));
                            g.DrawImage(bm,
                                destRect,
                                Math.Max(0, startDrawX - min.X) * sourceWidth / (max.X - min.X), 0,
                                imgWidth * sourceWidth / (max.X - min.X), sourceHeight,
                                GraphicsUnit.Pixel,
                                imageAttributes);
                        }
                        break;
                    case CurtainType.Left:
                        if(min.X<BarLocation.X)
                        {
                            int imgWidth = (int)(Math.Min(max.X,BarLocation.X)-min.X);
                            Rectangle destRect = new Rectangle((int)min.X, (int)max.Y, imgWidth, (int)(min.Y - max.Y));
                            g.DrawImage(bm,
                                destRect,
                                0, 0,
                                imgWidth * sourceWidth / (max.X - min.X), sourceHeight,
                ...
                //only draw images that's in right of bar
    ...
    public class CurtainLayer
    {
        Forms.MapBox _mapBox;
        CurtainTileLayer ctlL;
        CurtainTileLayer ctlR;
        public Point SplitLocation { get; private set; }

        public CurtainLayer(ITileSource sourceLeft,ITileSource sourceRight,string name,Forms.MapBox mb)
        {
            _mapBox = mb;
            ctlL = new CurtainTileLayer(sourceLeft, "curtainLayerLeft");
            ctlL.CT = CurtainTileLayer.CurtainType.Left;
            ctlR = new CurtainTileLayer(sourceRight, "curtainLayerRight");
            ctlR.CT = CurtainTileLayer.CurtainType.Right;
            CurtainTileLayer.BarColor = Color.White;
            CurtainTileLayer.BarLocation= new Point(_mapBox.Width / 2, 0);
            CurtainTileLayer.BarWidth = 3;
        }
        public void Add2Map()
        {
            //sometimes throw ThrowIfCancellationRequested, i have no idea
            _mapBox.Map.Layers.Add(ctlL);
            _mapBox.Map.Layers.Add(ctlR);
            BindMouseEvent();
            _mapBox.Refresh();
        }
        private void BindMouseEvent()
        {
            _mapBox.MouseDown += _mapBox_MouseDown;
            _mapBox.MouseMove += _mapBox_MouseMove;
            _mapBox.MouseUp += _mapBox_MouseUp;
        }
        bool isMouseDown = false;
        private void _mapBox_MouseUp(Coordinate worldPos, System.Windows.Forms.MouseEventArgs imagePos)
        {
            isMouseDown = false;
            _mapBox.ActiveTool = SharpMap.Forms.MapBox.Tools.Pan;
            _mapBox.Cursor = Cursors.Hand;
        }

        private void _mapBox_MouseMove(Coordinate worldPos, System.Windows.Forms.MouseEventArgs imagePos)
        {
            if (ctlL != null)
            {
                Rectangle barRect = new Rectangle(CurtainTileLayer.BarLocation.X, CurtainTileLayer.BarLocation.Y, CurtainTileLayer.BarWidth, _mapBox.Height);
                if (barRect.Contains(imagePos.X, imagePos.Y))
                {
                    _mapBox.Cursor = Cursors.VSplit;
                }
                if (isMouseDown)
                {
                    _mapBox.Cursor = Cursors.VSplit;
                    CurtainTileLayer.BarLocation = new Point(imagePos.X, CurtainTileLayer.BarLocation.Y);
                    System.Diagnostics.Debug.WriteLine(CurtainTileLayer.BarLocation.X);
                    ctlL.Render(_mapBox.CreateGraphics(), _mapBox.Map);
                    ctlR.Render(_mapBox.CreateGraphics(), _mapBox.Map);
                }
                else if (!isMouseDown && !barRect.Contains(imagePos.X, imagePos.Y))
                {
                    _mapBox.Cursor = Cursors.Hand;
                }
            }
        ...
        private void _mapBox_MouseDown(Coordinate worldPos, System.Windows.Forms.MouseEventArgs imagePos)
        {
            if (ctlL != null)
            {
                Rectangle barRect = new Rectangle(CurtainTileLayer.BarLocation.X, CurtainTileLayer.BarLocation.Y, CurtainTileLayer.BarWidth, _mapBox.Height);
        ...
        {
            _mapBox.Map.Layers.Remove(ctlL);
            _mapBox.Map.Layers.Remove(ctlR);
            ReleaseMouseEvent();
            _mapBox.Refresh();
        }
        private void ReleaseMouseEvent()
        {
            _mapBox.MouseDown -= _mapBox_MouseDown;
            _mapBox.MouseMove -= _mapBox_MouseMove;
            _mapBox.MouseUp -= _mapBox_MouseUp;
